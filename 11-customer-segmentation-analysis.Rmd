...
customers <- Online_Retail %>%
  mutate(date.diff = as.numeric(as.Date("2011-12-10") - as.Date(InvoiceDate))) %>% #計算購買或退貨日期與2011-12-10差幾天
  
  group_by(CustomerID, purchase.invoice) %>% #依據CustomerID與購買或退貨，將發票紀錄分組
  summarise(recency = min(date.diff),        #最近購買日期
            frequency=n_distinct(InvoiceNo), #購買頻率(一年內購買幾次)
            monetary=sum(Quantity*UnitPrice))#購買金額
```
